...
test('mobile rider flow has live routes, stops and stale-data affordances', async ({ page }) => {
  ...
  await expect(page.getByText('UGA estimates')).toBeVisible()
  await page.getByRole('button', { name: 'Routes', exact: true }).click()
  await expect(page.getByRole('heading', { name: 'Live bus lines' })).toBeVisible()
  await page.getByRole('button', { name: /^Orbit/ }).click()
  ...
  await expect(page.getByText('UGA estimates')).not.toBeVisible()
  await page.getByRole('button', { name: /^Orbit/ }).click()
  await expect(page.getByText('UGA estimates')).toBeVisible()
