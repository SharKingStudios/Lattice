...
install -m 0644 "$app_dir/infra/nest-proxy.Caddyfile" /etc/uga-bus/nest-proxy.Caddyfile
install -m 0644 "$app_dir/infra/uga-bus-proxy.service" /etc/systemd/system/uga-bus-proxy.service
systemctl daemon-reload
systemctl disable --now caddy.service || true
systemctl enable --now uga-bus.service uga-bus-backup.timer uga-bus-proxy.service
